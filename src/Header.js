    ...
    <div style={{
      ...
      <div className="flex items-center justify-between px-6 py-4">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <span style={{ backgroundColor: 'var(--text-muted)' }} className={`block w-5 h-0.5 transition-all ${menuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
            <span style={{ backgroundColor: 'var(--text-muted)' }} className={`block w-5 h-0.5 transition-all ${menuOpen ? 'opacity-0' : ''}`}></span>
            <span style={{ backgroundColor: 'var(--text-muted)' }} className={`block w-5 h-0.5 transition-all ${menuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
          </button>
        </div>
      </div>
      ...
      {menuOpen && (
        <div style={{ borderTop: '1px solid var(--border)' }} className="md:hidden px-6 py-4 flex flex-col gap-2">
          {['/', '/zakaznici', '/profil'].map((path, i) => (
            <Link
              key={path}
              to={path}
