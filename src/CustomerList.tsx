...
function CustomerList() {
  ...
  const handleSelectCustomer = async (customer: Customer): Promise<void> => {
    ...
    const response = await fetchWithAuth(
      `${API_URL}/api/customers/${customer.id}/detail`,
    );
                  ...
                  {customerDetail.upcoming.length > 0 && (
                    ...
                    >
                      <div
                        ...
                        className="font-medium text-xs mb-1 tracking-wide uppercase"
                      >
                      ...
                      {customerDetail.upcoming.map((u) => (
                        ...
                        >
                          {u.service_name} —{" "}
                          {new Date(u.appointment_date).toLocaleDateString(
                            "cs-CZ",
                          )}{" "}
                          v {u.appointment_time.slice(0, 5)}
                        </div>
                      ))}
                    </div>
                  )}
