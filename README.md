...
- podpora neregistrovaných zákazníků v termínech
...
Multi-tenant — každý podnikatel vidí jen svoje zákazníky a termíny. Veřejná část (vyhledávač, profily, booking) běží bez přihlášení. Frontend je kompletně přepsaný do TypeScriptu.

```
salon-booking/               frontend (Vercel)
├── src/
│   ├── App.tsx              routing, auth, theme
│   ├── Login.tsx            přihlášení
│   ├── Header.tsx           navigace
│   ├── AppointmentList.tsx  seznam termínů
│   ├── AddAppointment.tsx   přidání termínu
│   ├── CustomerList.tsx     zákazníci
│   ├── Profile.tsx          profil + služby + dostupnost + vzhled
│   ├── PublicSearch.tsx     vyhledávač salonů
│   ├── PublicProfile.tsx    veřejný profil + booking
│   └── config.ts            API URL + fetchWithAuth
...
Projekt aktivně používám jako testovací prostředí — průběžně přidávám funkce podle reálné zpětné vazby od uživatele. Frontend je kompletně přepsaný do TypeScriptu.

---

_Student softwarového vývoje — Unicorn University | [GitHub](https://github.com/Darkstek)_
